"""
ICT V2 - 5min Timeframe Backtest (Past 2 Days)
...
print("=" * 80)
print("ICT V2 - 5MIN TIMEFRAME BACKTEST (2 DAYS)")
print("=" * 80)

def run_5min_backtest(symbol, days=2):
    print(f"\n{'='*80}")
    print(f"BACKTEST: {symbol} (5min, {days} days)")
    print(f"{'='*80}")
    ...
    try:
        ...
        if df.empty or len(df) < 50:
            print(f"Not enough 5min data for {symbol}")
            return None
    ...
    print(f"Data: {len(df)} 5min bars from {df.index[0]} to {df.index[-1]}")
    ...
    # LTF Trend (20 bar lookback on 5min = 1.6 hours)
    ...
    for idx in range(30, len(df)):
        ...
        # Count all A-grade signals (even if not entered)
        if confluence >= 70:
            signals_found += 1
        ...
        # ATR on 5min
        atr = (highs[max(0,idx-14):idx] - lows[max(0,idx-14):idx]).mean() if idx > 14 else 10
        ...
        if open_trade:
            ...
            # Time exit (180 bars = 15 hours on 5min)
...
for symbol in symbols:
    result = run_5min_backtest(symbol, days=2)
    if result:
        all_results.append(result)

# Summary
print(f"\n{'='*80}")
print("5MIN TIMEFRAME SUMMARY - PAST 2 DAYS")
print(f"{'='*80}")
...
output = {
    'metadata': {
        'strategy': 'ICT V2 (5min, Max 1 Contract)',
        'timeframe': '5min',
        'period': 'past_2_days',
...
with open('v2_5min_2day_trades.json', 'w') as f:
    json.dump(output, f, indent=2)

print(f"\nDetailed trades saved to: v2_5min_2day_trades.json")
