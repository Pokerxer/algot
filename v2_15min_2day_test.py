"""
ICT V2 - 15min Timeframe Backtest (Past 2 Days)
...
print("=" * 80)
print("ICT V2 - 15MIN TIMEFRAME BACKTEST (2 DAYS)")
print("=" * 80)

def run_15min_backtest(symbol, days=2):
    print(f"\n{'='*80}")
    print(f"BACKTEST: {symbol} (15min, {days} days)")
    print(f"{'='*80}")
    
    capital = 10000
    
    # Fetch 15min data (Yahoo limits to 60 days for 15min)
    print(f"Fetching {symbol} 15min data...")
    try:
    ...
    df_hourly = yf.Ticker(symbol).history(period=f"{days}d", interval="1h")
    
    print(f"Data: {len(df)} 15min bars from {df.index[0]} to {df.index[-1]}")
    ...
    # Map hourly trend to 15min bars
    ...
    # LTF Trend (20 bar lookback on 15min = 5 hours)
    ...
    # Kill Zones (adjusted for 15min - more precise)
    ...
    for idx in range(30, len(df)):
        ...
        # ATR on 15min (14 bars = 3.5 hours)
        atr = (highs[max(0,idx-14):idx] - lows[max(0,idx-14):idx]).mean() if idx > 14 else 20
        ...
        if open_trade:
            ...
            # Time exit (60 bars = 15 hours on 15min)
            if open_trade and (idx - open_trade['entry_idx']) > 60:
        ...
        if idx % 100 == 0:
            print(f"  Progress: {idx}/{len(df)} | Trades: {len(trades)}")
    ...
    return {
        ...
        'pnl': total_pnl,
        'profit_factor': pf,
        'longs': {'count': len(longs), 'pnl': sum(t['pnl'] for t in longs)},
        'shorts': {'count': len(shorts), 'pnl': sum(t['pnl'] for t in shorts)},
...
# Run for all symbols
print("="*80)
print("STARTING 15MIN BACKTESTS")
print("="*80)
...
for symbol in symbols:
    result = run_15min_backtest(symbol, days=2)
    if result:
        all_results.append(result)

# Summary
print(f"\n{'='*80}")
print("15MIN TIMEFRAME SUMMARY - PAST 2 DAYS")
print(f"{'='*80}")
print(f"{'Symbol':<15} | {'Trades':<8} | {'Win%':<8} | {'PnL':<12} | {'PF':<6}")
print("-" * 70)
total_trades = 0
total_pnl = 0
for r in all_results:
    print(f"{r['symbol']:<15} | {r['trades_count']:<8} | {r['win_rate']:<8.1f} | ${r['pnl']:+>11,.0f} | {r['profit_factor']:<6.2f}")
    total_trades += r['trades_count']
    total_pnl += r['pnl']

print(f"{'='*80}")
print(f"TOTAL: {total_trades} trades | ${total_pnl:+,.0f}")
print(f"{'='*80}")

# Save results
output = {
    'metadata': {
        'strategy': 'ICT V2 (15min, Max 1 Contract)',
        'timeframe': '15min',
        'period': 'past_2_days',
...
with open('v2_15min_2day_trades.json', 'w') as f:
    json.dump(output, f, indent=2)

print(f"\nDetailed trades saved to: v2_15min_2day_trades.json")
...
if all_results:
    ...
    for r in all_results:
        if r['trade_details']:
            print(f"\n{r['symbol']}:")
            print(f"{'#':<3} | {'Entry Time':<16} | {'Dir':<5} | {'Entry':<8} | {'SL':<8} | {'TP':<8} | {'Exit':<8} | {'PnL':>10} | {'Status'}")
            print("-" * 100)
            for i, t in enumerate(r['trade_details']):
                print(f"{i+1:<3} | {t['entry_time'][:10]} {t['entry_time'][11:16]:<6} | {t['dir'].upper():<5} | {t['entry']:<8.2f} | {t['sl']:<8.2f} | {t['tp']:<8.2f} | {t.get('exit',0):<8.2f} | ${t['pnl']:+>9,.0f} | {t['status']}")
