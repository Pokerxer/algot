"""
V5 Backtest - Top 8 Performing Symbols
=====================================
Run backtest with only the best performers from previous results.
Top 8: BTCUSD, ES, ETHUSD, GC, NQ, SI, NG, CL
"""
        ...
        for idx in range(50, len(closes) - 1):
            ...
            if position is None and confluence >= 60:
                ...
                if stop_distance > 0:
                    ...
                    # For futures and crypto - simplified position sizing
...
# Top performing symbols + Crypto
# Adding: LTCUSD, LINKUSD, UNIUSD
top_symbols = ['BTCUSD', 'ETHUSD', 'SOLUSD', 'LTCUSD', 'LINKUSD', 'UNIUSD', 'ES', 'NQ', 'GC', 'SI', 'NG', 'CL']

print("="*80)
print("ICT V5 Backtest - Top 8 Performing Symbols (6 Months)")
print("="*80)
print(f"Symbols: {', '.join(top_symbols)}")
print("="*80)

results = run_backtest_json(top_symbols, days=180, risk_per_trade=0.02, use_ibkr=True)

# Save to JSON file
output_file = '/Users/mac/Documents/Algot/v5_top8_backtest_results.json'
with open(output_file, 'w') as f:
    json.dump(results, f, indent=2)

print(f"\n{'='*80}")
print("BACKTEST SUMMARY - TOP 8 SYMBOLS")
print(f"{'='*80}")
